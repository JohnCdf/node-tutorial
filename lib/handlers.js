      ...
      if(user){
        if (helpers.hash(password) == user.password) {
          ...
          lib.create(['tokens', tokenObj.key], tokenObj, function (err) {
            if (err) {
              callback(500, {
                message: 'There was an error creating this token'
              }, 'application/json');
            } else {
...
handlers._tokens.delete = function(data,callback){
  var id = typeof(data.payload.tokenId) == 'string' ? data.payload.tokenId.trim() : false;
  console.log('deteling ' + data.payload.tokenId)
  if(id){
