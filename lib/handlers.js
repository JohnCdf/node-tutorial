...
handlers.index = function(data, callback){
  if(data.method.toLowerCase() != 'get'){ 
    callback(405, {message:'That method is not allowed'}, 'application/json')
  } else {

    let templateData = {//for interpolation of the html
      'head.title' : 'Home'
    };

    helpers.getTemplate('index', templateData, function (err, string) {
      if(err){
        callback(500, {message:'There was an error loading this template'}, 'application/json')
      } else {
...
handlers.sessionTerminate = function ( data, callback ) {
      ...
      lib.read(['users', phone], function (err, user) {//reads user data ->
        if (err) {
          ...
          if (helpers.hash(password) != user.password) {//compares password ->
            ...
            if(agreement){
              callback(403,{message:'Cannot change agreement'}, 'application/json')
              return
            }
    ...
    lib.read(['users', phone], function (err, user) {
      if (helpers.hash(password) == user.password) {
        if(user.token){
          ...
          callback(404, {message: 'Not found'}, 'application/json')
        }
...
handlers._tokens.delete = function (data,callback){
  var tokenId= typeof(data.token) == 'string' ? data.token : false;

  if(tokenId){
    lib.delete('tokens',tokenId, function (err) {
      if(err){
        console.log(err)
      }
      callback(200,{message:'Deleted!'},'application/json')
    })
  } else {
    callback(400, {})
  }
...
handlers.notfound = function (data, callback) {
  callback(404, 'The route you requested does not exist. How sad.');
};
