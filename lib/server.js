...
var unifiedServer = function(request, response){
    ...
    request.on('end',function(){//Select handler and pass on the data
        ...
        var handler = typeof(router[trimmedPath]) !== 'undefined' ? router[trimmedPath] : handlers.notfound;//if route doesnt exist, its 404
        handler = trimmedPath.indexOf('public/') > -1 ? handlers.public : handler;
        for ( route in router ) {//If our route is in the routes
